...
# Source: Miao et al. 2020
pca_fun = function(Y.vec,T.vec, Xmat,bhat,K,n,BS,basis,Omega,Omega2,NumObsPerLine,J)
{
##################################################################################
# Estiamtion of G1
##################################################################################
...
# the covariates part
BS.i = eval.basis(T.vec[(start.temp:(start.temp+n.i-1))],basis,0)
...
bhat.G1 = Matrix::solve(t(Xmat.tensor)%*%Xmat.tensor+lam.G1*Omega2) %*%t(Xmat.tensor)%*%ee.vec
